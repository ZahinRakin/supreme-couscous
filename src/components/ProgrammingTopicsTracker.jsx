...
const topics = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Basic Search/Sorting algorithm", link: "https://visualgo.net/en/sorting" },
  ...
  },
  {
    category: "Puzzle",
    items: [
      { name: "Simple puzzle", link: "https://leetcode.com/problemset/all/?difficulty=Easy" },
...
export default function ProgrammingTopicsTracker() {
  ...
  useEffect(() => {
    if (!isLoading) {
      try {
        localStorage.setItem('programmingTopicsProgress', JSON.stringify(checkedItems));
      } catch (error) {
  ...
  const totalItems = topics.reduce((sum, section) => sum + section.items.length, 0);
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 p-6">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-gray-600">Master your programming journey, one topic at a time</p>
          ...
          <div className="max-w-md mx-auto">
            ...
            <p className="text-sm text-gray-500 mt-2">{completedItems} of {totalItems} topics completed</p>
          </div>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {topics.map((section, i) => {
            ...
            return (
              <Card key={i} className={`${section.bgColor} border border-white/50 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1`}>
                <CardContent>
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="text-right">
                        ...
                        <div className={`text-2xl font-bold bg-gradient-to-r ${section.color} bg-clip-text text-transparent`}>
                          {progress}%
                        </div>
                      </div>
                    </div>
        ...
        {overallProgress === 100 && (
          <div className="text-center p-8 bg-gradient-to-r from-yellow-400 to-orange-500 rounded-3xl shadow-2xl">
            ...
            <p className="text-white/90 text-lg">You've completed all programming topics! You're ready to tackle advanced challenges!</p>
          </div>
        )}
      </div>
    </div>
